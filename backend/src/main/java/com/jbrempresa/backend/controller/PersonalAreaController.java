...
public class PersonalAreaController {
 ...
 @GetMapping public List<PersonalSalida> consultar(){return (contexto.administradorGlobal()?repository.findAll():repository.findByEmpIdOrderByPeaId(contexto.empresaId())).stream().map(personal->PersonalSalida.desde(personal)).toList();}
 ...
 @PutMapping("/{id}") public PersonalSalida actualizar(@PathVariable Long id,@Valid @RequestBody PersonalEntrada datos){PersonalArea p=datos.entidad();PersonalArea existente=personalPermitido(id);p.setEmpId(existente.getEmpId());p.setPeaId(id);preparar(p);validar(p,id);return PersonalSalida.desde(repository.save(p));}
 @DeleteMapping("/{id}") public void eliminar(@PathVariable Long id){repository.delete(personalPermitido(id));}
 private void preparar(PersonalArea p){if(!contexto.administradorGlobal()||p.getEmpId()==null||p.getEmpId()==0)p.setEmpId(contexto.empresaId());p.setPeaUsuMov(contexto.nombreUsuario());p.setPeaFecMov(contexto.fechaActual());if(p.getPeaAct()==null)p.setPeaAct(true);if(p.getPeaPri()==null)p.setPeaPri(false);if(p.getPeaRes()==null)p.setPeaRes(false);}
 private PersonalArea personalPermitido(Long id){PersonalArea p=repository.findById(id).orElseThrow(()->new RuntimeException("Asignación no encontrada."));if(!contexto.administradorGlobal()&&!p.getEmpId().equals(contexto.empresaId()))throw new RuntimeException("Asignación no encontrada.");return p;}

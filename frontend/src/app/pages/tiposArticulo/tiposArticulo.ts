...
export class TiposArticulo implements OnInit {
 ...
 columnas=['id','nombre','usuario','fecha','activo'];titulosColumnas={id:'Id Tipo',nombre:'Nombre',usuario:'Usuario Mod.',fecha:'Fecha Mod.',activo:'Activo'};
 constructor(private api:TipoArticuloService,private route:ActivatedRoute,private router:Router){} ngOnInit(){this.clase=this.route.snapshot.data['clase'];this.modulo=this.clase==='PRODUCTO'?'productos':'servicios'} get titulo(){return`REGISTRO DE TIPOS DE ${this.clase}`}

# Manual de parámetros por empresa

Revisión: 2026-09-15. Los nombres de parámetros se contrastan con el código; los menús y condiciones del proveedor externo deben verificarse en su cuenta.

## 1. Finalidad

Los parámetros permiten adaptar el comportamiento de cada empresa sin modificar el código. Cada registro pertenece a una única empresa (`emp_id`) y queda identificado por la combinación de módulo (`par_mod`) y código (`par_cod`).
...
| Valor | Configuración efectiva para el empresa. |
...
- Debe existir como máximo un parámetro con el mismo módulo y código para cada empresa.
- No se deben copiar valores secretos entre empresas.
- Las rutas deben pertenecer al empresa y ser accesibles por el proceso del backend.
...
- Los parámetros de empresa se consultan desde base de datos. Cambiar variables del entorno del proceso requiere reiniciar el backend; cambiar un parámetro no exige por sí solo reiniciarlo.
...
| `SMTP_USUARIO` | Cuenta usada para autenticarse. | Cuenta de correo creada para el empresa. | `avisos@empresa.es` |
...
Cada ruta indica el directorio raíz donde el backend guarda los adjuntos de la empresa. La aplicación añade internamente el tipo de registro y su identificador.

| Módulo | Código | Utilidad | Cómo establecer el valor |
|---|---|---|---|
| `PERSONAS` | `RUTA_DOCUMENTOS_PERSONAS` | Adjuntos de personas. | Crear una carpeta exclusiva de la empresa con permisos de lectura y escritura para el backend. |
| `PRODUCTOS` | `RUTA_DOCUMENTOS_PRODUCTOS` | Adjuntos de productos. | Igual que el anterior, dentro del área de productos. |
| `SERVICIOS` | `RUTA_DOCUMENTOS_SERVICIOS` | Adjuntos de servicios. | Debe crearse para usar la acción Adjuntos de Servicios. Comprobar que existe y está activo en la empresa. |
| `TERRITORIO` | `RUTA_DOCUMENTOS_DOMICILIOS` | Adjuntos de domicilios. | Carpeta exclusiva de la empresa para domicilios. |
| `VENTAS` | `RUTA_DOCUMENTOS_VENTAS` | Adjuntos de pedidos, albaranes, presupuestos y facturas. | Carpeta exclusiva de la empresa para ventas. |
| `COMUNICACIONES` | `RUTA_DOCUMENTOS_COMUNICACIONES` | Documentos e imágenes de mensajes y conversaciones. | Carpeta exclusiva de la empresa para comunicaciones. |
...
No se debe apuntar a la misma carpeta para empresas diferentes.
...
|---|---|---|
| `WHATSAPP_PROVEEDOR` | Decide qué integración procesa los envíos de WhatsApp de la empresa. | `TWILIO` o `META`. |
...
| `TWILIO_WHATSAPP_FROM` | Remitente de WhatsApp asignado por Twilio. También permite identificar al empresa receptor del webhook. | En pruebas aparece en **Try out WhatsApp**. En producción se obtiene al registrar el remitente. Guardar con `whatsapp:+` y prefijo internacional. |
...
| `WHATSAPP_WEBHOOK_VERIFY_TOKEN` | Texto secreto elegido por el administrador para verificar inicialmente el webhook. Es sensible. | Generar una cadena aleatoria y configurar exactamente la misma en Meta y en el empresa. |
...
3. Confirmar el proveedor efectivo; reiniciar solo si se han cambiado variables de entorno.
...
## 11. Imágenes, módulos y pagos

RUTA_IMAGENES se configura en ADMINISTRACION, PRODUCTOS y SERVICIOS con la raíz común de imágenes de la empresa. El backend añade empresa y tipo. RUTA_DOCUMENTOS_MODULOS, en ADMINISTRACION, guarda los adjuntos del maestro de módulos. Véase [ficheros e imágenes](../backend/docs/FICHEROS_IMAGENES.md).

Los ocho parámetros PAGO_BIZUM_* se detallan en [Configuración](../backend/docs/CONFIGURACION.md). Son preparación de la integración; activarlos no implementa un pago real. La clave de firma se cifra y enmascara. El mismo documento explica REQUERIR_CLAVE_MODIFICACION_CADENA y CLAVE_MODIFICACION_CADENA; esta última es una clave operativa y no comparte la protección de credenciales.
